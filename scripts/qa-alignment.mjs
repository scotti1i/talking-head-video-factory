...
//   任一锚点偏差 > 1 帧或相关度 < 0.5 即失败。同时暴露 concat 帧量化累积漂移和倍速 / 母版错位（2026-09-11 审计机制 1 与 3）。
...
function pcm(file, start, length, extraAudioFilter) {
  const args = ["-v", "error", "-ss", String(Math.max(0, start)), "-t", String(length), "-i", file, "-vn", "-ac", "1", "-ar", String(SR), ...(extraAudioFilter ? ["-af", extraAudioFilter] : []), "-f", "f32le", "-"];
...
function main() {
  ...
  for (const anchor of anchorsFromEdl(segments, rate)) {
    ...
    const { lagSamples, score } = bestLag(ref.subarray(0, Math.round(REF_SECONDS * SR)), target);
    const measured = Math.max(0, windowStart) + lagSamples / SR;
    const offset = measured - anchor.expectedOutput;
    const ok = score >= 0.5 && Math.abs(offset) <= toleranceSeconds;
    items.push({ ...anchor, measuredOutput: round(measured), offsetSeconds: round(offset), offsetFrames: round(offset * fps), score: round(score), ok });
  }
  ...
  for (const item of failed) console.log(`  ✗ 段 ${item.index + 1} ${item.source} 偏差 ${item.offsetFrames ?? "?"} 帧 相关 ${item.score ?? "?"} ${item.error || ""}`);
