...
import { escapeHtml, fmtTime, readJson, writeJson } from "./lib.mjs";

const root = path.resolve(new URL("..", import.meta.url).pathname);
const sourceJob = path.join(root, "jobs", "dji-20260704-douyin");
const job = path.join(root, "jobs", "dji-20260704-douyin-claude-glass");
const duration = 255.734;
...
writeJson(path.join(job, "project.json"), {
  ...
  slug: "dji-20260704-douyin-claude-glass",
  platform: "douyin",
  width: 1080,
  height: 1920,
  duration,
  sourceVideo: "assets/aroll.mp4",
  style: "talkinghead-edit claude warm glass dynamic ppt",
  outputName: "final-claude-glass-60fps.mp4",
  downloadFolderName: "2026-07-04-AI不是给普通员工提效-抖音成片-Claude动态卡版"
});
fs.writeFileSync(path.join(job, "package.json"), `${JSON.stringify({
  name: "talking-head-job-claude-glass",
  private: true,
  type: "module",
  scripts: {
    ...
    "render:review": "npx --yes hyperframes@0.5.6 render --fps 30 --quality draft --workers 4 --video-bitrate 8M --output renders/review-claude-glass-30fps.mp4",
    "render:final": "npx --yes hyperframes@0.5.6 render --fps 60 --quality standard --workers 8 --video-bitrate 24M --output renders/final-claude-glass-60fps.mp4"
  },
  dependencies: {
    hyperframes: "0.5.6"
  }
