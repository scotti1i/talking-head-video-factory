...
const job = path.join(root, "jobs", "dji-20260704-dynamic-tight");
const oldEdl = readJson(path.join(sourceJob, "data", "rough-cut-edl.json"));
const newEdl = readJson(path.join(job, "data", "rough-cut-edl.json"));
const duration = videoDuration(path.join(job, "assets", "aroll.mp4"));

const sourceBeats = [
  { type: "statement", start: 0.0, end: 5.2, kicker: "开场判断", title: "AI 不会平均提效", body: "它放大的不是努力，是人的底层能力差。", accent: "绝对不可能" },
  { type: "hero", start: 5.4, end: 15.9, kicker: "触发点", title: "不是员工懒", number: "WHY", body: "是工具把能力差直接照出来了。" },
  ...
  { type: "statement", start: 45.4, end: 57.6, kicker: "分水岭", title: "AI 会先淘汰协作成本", body: "不是谁都会被提效，先被放大的是强弱差距。", accent: "差距放大" },
  ...
  { type: "chips", start: 82.5, end: 92.7, kicker: "真正浪费", title: "AI 省下来的时间，又被解释吃掉", chips: ["讲需求", "补背景", "反复确认", "返工"], body: "个人快，不等于团队快。" },
  ...
  { type: "duel", start: 123.4, end: 129.2, kicker: "不要误判", title: "内容工具，不等于商业系统", bad: "套模板", good: "重流程", body: "UGC 能自动化，不代表商单可以乱来。" },
  ...
  { type: "chips", start: 193.0, end: 213.0, kicker: "Claude Code 时代", title: "需求讲清 + Token 给够，Agent 才能跑", chips: ["Opus 4", "Sonnet 4", "上下文", "执行预算"], body: "真正的门槛，是你能不能把需求说清楚。" },
...
const hideRanges = beats.map((beat) => ({ start: beat.start, end: beat.end }));
...
function renderCaptions() {
  return captions
    .map((item, index) => {
      ...
      if (overlaps(start, end, hideRanges)) return "";
...
function renderMotionLayer(beat, index) {
  ...
  return `<video id="${id}" class="clip motion-video motion-${beat.type}" src="assets/aroll.mp4" data-start="${start}" data-duration="${durationText}" data-media-start="${mediaStart}" data-track-index="${20 + index}" muted playsinline preload="auto" style="--clip-dur:${durationText}s"></video>
...
<html>
  ...
  <body>
    <div id="main" data-composition-id="main" data-width="1080" data-height="1920" data-start="0" data-duration="${fmtTime(duration)}">
      <div id="video-wrap">
        <video id="talking-video" src="assets/aroll.mp4" data-start="0" data-duration="${fmtTime(duration)}" data-track-index="1" muted playsinline preload="auto"></video>
      </div>
      <audio id="talking-audio" src="assets/aroll.mp4" data-start="0" data-duration="${fmtTime(duration)}" data-track-index="2" preload="auto"></audio>
      <div id="scrim"></div>
    ...
    <script>
      ...
      document.querySelectorAll(".beat").forEach((card) => {
        ...
        tl.from(card, { y: 42, scale: 0.965, rotate: -0.3, duration: 0.45, ease: "expo.out" }, start + 0.08);
        tl.to(card, { y: -20, duration: 0.24, ease: "power3.in" }, start + Math.max(0.8, dur - 0.30));
        tl.from(card.querySelectorAll("h2, p, strong, .panel-list span, .chip-grid span, .split-side, .duel-row div, .pipeline div, .trio span, .node, .cta-row span"), { y: 20, opacity: 0, duration: 0.32, stagger: 0.055, ease: "power3.out" }, start + 0.20);
        tl.to(scrim, { opacity: kind === "split" || kind === "panel" ? 0.52 : 0.68, duration: 0.28, ease: "sine.out" }, start);
        tl.to(scrim, { opacity: 0, duration: 0.28, ease: "sine.in" }, start + Math.max(0.8, dur - 0.34));
        const transform = kind === "split" || kind === "panel"
          ? { x: 205, y: -18, scale: 0.86, rotate: 0.35, duration: 0.40, ease: "expo.out" }
          : kind === "hero" || kind === "statement"
            ? { x: 0, y: -18, scale: 1.055, rotate: 0, duration: 0.38, ease: "expo.out" }
            : { x: 0, y: -8, scale: 1.02, rotate: -0.18, duration: 0.38, ease: "expo.out" };
        tl.to(videoWrap, transform, start);
        tl.to(videoWrap, { x: 0, y: 0, scale: 1, rotate: 0, duration: 0.34, ease: "power3.inOut" }, start + Math.max(0.9, dur - 0.38));
      });
...
writeJson(path.join(job, "project.json"), {
  ...
  slug: "dji-20260704-dynamic-tight",
  platform: "douyin",
  width: 1080,
  height: 1920,
  duration,
  sourceVideo: "assets/aroll.mp4",
  style: "talkinghead-edit claude warm glass dynamic tight cut",
  outputName: "final-dynamic-tight-60fps.mp4",
  downloadFolderName: "2026-07-04-AI不是给普通员工提效-抖音成片-动态紧剪版"
});
fs.writeFileSync(path.join(job, "package.json"), `${JSON.stringify({
  name: "talking-head-job-dynamic-tight",
  private: true,
  type: "module",
  scripts: {
    ...
    "render:review": "npx --yes hyperframes@0.5.6 render --fps 30 --quality draft --workers 4 --video-bitrate 8M --output renders/review-dynamic-tight-30fps.mp4",
    "render:final": "npx --yes hyperframes@0.5.6 render --fps 60 --quality standard --workers 8 --video-bitrate 24M --output renders/final-dynamic-tight-60fps.mp4"
  },
  dependencies: {
    hyperframes: "0.5.6"
  }
