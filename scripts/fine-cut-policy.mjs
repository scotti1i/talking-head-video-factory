...
export function resolveFineCutPreset(project, registry = loadFineCutRegistry()) {
  const profileDefault = registry.profileDefaults?.[String(project?.profile || "").trim()];
  const requested = String(project?.editorial?.fineCutPreset || profileDefault || registry.default).trim();
...
function validateRegistry(registry, file) {
  ...
  for (const [profile, presetId] of Object.entries(registry.profileDefaults || {})) {
    if (!registry.presets[presetId]) throw new Error(`${file}: profileDefaults.${profile} 指向未知 preset ${presetId}`);
  }
